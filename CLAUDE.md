...
- **`listen/`** — Socket Mode event loop. `Listener` wraps `socketmode.Client`, filters self-events by bot user ID, and emits typed `Event` structs as JSONL to stdout. Status messages go to stderr. Event types: `mention`, `dm`, `reaction_added`, `reaction_removed`, `thread_reply` (emitted when `--threads` or `--all-messages` is set), `channel_message` (emitted only with `--all-messages`).
...
`slackline provision NAME` (per-bot, admin): rotates the config token, calls `apps.manifest.create`, and writes machine-readable JSON to stdout with `app_id`, `install_url`, `oauth_authorize_url`, `oauth_page_url`, `general_page_url`. No interactive prompts. The agentic recipe for driving the browser install + token-collection steps lives in the `slackline-provision-bot` skill (`.claude-plugin/` + `skills/slackline-provision-bot/SKILL.md`).
