...
- **`listen/`** — Socket Mode event loop. `Listener` wraps `socketmode.Client`, filters self-events by bot user ID, and emits typed `Event` structs as JSONL to stdout. Status messages go to stderr. Event types: `mention`, `dm`, `reaction_added`, `reaction_removed`, `thread_reply` (emitted by default for replies in threads the bot started; `--all-messages` widens to all thread replies), `channel_message` (emitted only with `--all-messages`). `--threads` is accepted for backward compatibility but is a no-op since v0.2.1 — bot-parent thread replies are always emitted.
