...
- **`errs/`** — `SlackError` type with `Code` field (`Success`/`SlackAPI`/`Auth`/`Config`/`Usage`/`Timeout` = 0–5). `WriteError` writes `{"error":"...","detail":"..."}` to stderr. `Execute()` in root maps returned errors to exit codes.
