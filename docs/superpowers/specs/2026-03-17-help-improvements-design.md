...
### 1. Bug Fix — `Execute()` in `cmd/root.go`
...
Test requirement: invoke a command (`ask`, `send`, or `read`) without its required `--channel` flag. Assert exit code 4. Assert stderr contains JSON with `"error":"usage_error"`. This test locks the behavior and will catch cobra message changes.
...
The `detail` value is constructed as `err.Error() + " — run 'slackline <cmd> --help' for usage"`. The command name comes from `rootCmd.Execute()`'s context — use `err.Error()` as-is and append the suffix without substituting the command name (cobra's message already names the flag). Return **exit code 4** (`errs.Usage`).
...
Exit codes: 0 success, 1 Slack API error, 2 auth failure, 3 config error, 4 usage error
```
...
Missing message is caught at runtime with exit code 4.

Output: {"ok":true,"channel":"C...","ts":"..."}
Exit codes: 0 success, 1 Slack API error, 2 auth, 3 config, 4 usage
...
Exit codes: 0 reply received, 1 timeout or Slack API error, 2 auth, 3 config, 4 usage
...
Exit codes: 0 clean exit, 1 connection error, 2 auth, 3 config
...
Exit codes: 0 success, 1 Slack API error, 2 auth, 3 config
...
unless the config file is missing or unreadable (exit 3). Exit codes 1 and 2 are never returned.
...
Exit codes: 0 success, 2 auth, 3 config, 4 usage
